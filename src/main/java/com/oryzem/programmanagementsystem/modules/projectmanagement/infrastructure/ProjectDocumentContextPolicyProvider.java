...
public class ProjectDocumentContextPolicyProvider implements DocumentContextPolicyProvider {
    ...
    public ProjectDocumentContextPolicyProvider(
    ...
    public DocumentContextPolicy resolve(String contextId, AuthenticatedUser actor) {
        ProjectEntity project = projectRepository.findById(contextId).orElse(null);
        if (project == null) {
            return new DocumentContextPolicy(false, false, false, null, false, false, false, false);
        }
        var organizations = organizationRepository.findAllByProjectIdAndActiveTrueOrderByJoinedAtAsc(contextId).stream()
                ...
                .toList();
        var members = memberRepository.findAllByProjectIdAndActiveTrueOrderByAssignedAtAsc(contextId).stream()
        ...
        return new DocumentContextPolicy(
                ...
                authorizationService.canAccessProject(project.toDomain(), organizations, members, actor, ProjectPermission.VIEW_DOCUMENT),
                authorizationService.canAccessProject(project.toDomain(), organizations, members, actor, ProjectPermission.UPLOAD_DOCUMENT),
                authorizationService.canAccessProject(project.toDomain(), organizations, members, actor, ProjectPermission.DOWNLOAD_DOCUMENT),
                authorizationService.canAccessProject(project.toDomain(), organizations, members, actor, ProjectPermission.DELETE_DOCUMENT));
    }
}
