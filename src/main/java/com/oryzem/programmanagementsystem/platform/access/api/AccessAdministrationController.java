...
public class AccessAdministrationController {
    ...
    @PatchMapping("/tenants/{tenantId}/service-tier")
    public TenantServiceTierChangeResponse changeTenantServiceTier(
            Authentication authentication,
            @PathVariable String tenantId,
            @Valid @RequestBody ChangeTenantServiceTierRequest request) {
        AuthenticatedUser actor = authenticatedUserMapper.from(authentication);
        return accessAdministrationService.changeTenantServiceTier(actor, tenantId, request);
    }
}
