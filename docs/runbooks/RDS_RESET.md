...
- keep only `vanderson.verza@gmail.com.br` as the initial internal admin user

Preferred path:
1. Confirm the target is the development RDS instance.
2. Make sure no one else is using the environment.
3. Run `scripts/reset-rds-safe.ps1`.
...
- `vanderson.verza@gmail.com.br` exists as the internal admin bootstrap user
