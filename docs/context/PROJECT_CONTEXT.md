...
- the project detail screen should also expose runtime project structure nodes so operators can create concrete items under configured structure levels, for example creating `Parts` children under the root `Project` node and then viewing the newly materialized deliverables scoped by that node
